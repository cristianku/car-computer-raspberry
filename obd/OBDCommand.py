...
class OBDCommand():
    ...
    def __call__(self, messages):
        ...
        if messages:
            ...
            print(str(self) + " did not recieve any acceptable messages")
    ...
    def __constrain_message_data(self, message):
        ...
        if self.bytes > 0:
            if len(message.data) > self.bytes:
                ...
                print("Message was longer than expected. Trimmed message: " + repr(message.data))
            elif len(message.data) < self.bytes:
                # pad the right with zeros
                message.data += (b'\x00' * (self.bytes - len(message.data)))
                print("Message was shorter than expected. Padded message: " + repr(message.data))
