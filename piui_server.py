...
class DemoPiUi(object):
    ...
    def page_buttons(self):
        ...
        recording = self.page.add_button("Start Video Recording", self.onupclick)
        camera = self.page.add_button("Show Camera", self.cameraclick)
        minus = self.page.add_button("Shutdown now", self.rebootclick)
    ...
    def rebootclick(self):
        self.title.set_text("Shutting down")
        print ("Shutting down")
        from subprocess import Popen, PIPE
        Popen("sudo shutdown now", stdout=PIPE, stderr=PIPE, shell=True)

    def cameraclick(self):
        self.page = self.ui.new_ui_page(title="Camera", prev_text="Back", onprevclick=self.main_menu)
        self.title = self.page.add_textbox("camera_still!", "h1")
        from subprocess import Popen, PIPE
        Popen("sudo cp /home/pi/python/camera_still.jpg /imgs", stdout=PIPE, stderr=PIPE, shell=True)
        self.page.add_image("camera_still.jpg")
